...
class EmailService {
    static async sendContactConfirmation(to, name) {
        try {
            await email_1.resend.emails.send({
                from: `${email_1.emailConfig.from.name} <onboarding@resend.dev>`,
                to,
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <br>
      <p>Best regards,</p>
      <p><strong>${email_1.emailConfig.from.name}</strong></p>
  ...
  `,
    ...
    static async sendContactNotification(contact) {
        try {
            await email_1.resend.emails.send({
                from: `${email_1.emailConfig.from.name} <onboarding@resend.dev>`,
                to: process.env.FROM_EMAIL || "deyg6988@gmail.com",
